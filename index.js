...
function setToArray(mySet) {
  ...
  const filteredArray = array.filter((item) => typeof item === "string"); // Фільтруємо масив, залишаючи лише рядкові елементи
  const sortedArray = filteredArray.sort(); // Сортуємо рядкові елементи в алфавітному порядку
